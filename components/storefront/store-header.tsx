      ...
      >
        ...
        <div className="p-4 bg-gray-50 border-b border-gray-100 shrink-0">
          <div className="flex bg-white rounded-xl p-1 shadow-sm border border-gray-100">
            <button 
              onClick={() => switchCart('now')}
              className={`flex-1 py-2 text-sm font-bold rounded-lg transition-colors ${activeCartType === 'now' ? 'bg-[#ffc000] text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-50'}`}
            >
              Order for Now
            </button>
            <button 
              onClick={() => switchCart('future')}
              className={`flex-1 py-2 text-sm font-bold rounded-lg transition-colors ${activeCartType === 'future' ? 'bg-[#ffc000] text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-50'}`}
            >
              Future Order
            </button>
          </div>
                ...
                <div key={item.id} className="flex gap-4 bg-white p-3 rounded-2xl border border-gray-100 shadow-sm">
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    ...
                    <div className="flex items-center justify-between mt-2 gap-2">
                      <span className="font-black text-gray-900 shrink-0">Rs. {item.price}</span>
                      ...
                      <button 
                        onClick={() => moveItemToCart(item.id, activeCartType === 'now' ? 'future' : 'now')}
                        className="text-[10px] font-bold text-gray-500 hover:text-gray-900 underline underline-offset-2 transition-colors whitespace-nowrap"
                      >
                        Move to {activeCartType === 'now' ? 'Future' : 'Now'}
                      </button>
                      </div>
                    </div>
                  </div>
        ...
        <div className="p-4 bg-white border-t border-gray-100 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
          ...
          {mounted && items.length > 0 ? (
            ...
            >
              Checkout {activeCartType === 'now' ? 'Now' : 'Later'} <ChevronRight className="w-5 h-5" />
            </Link>
          ) : (
            <button 
              disabled
