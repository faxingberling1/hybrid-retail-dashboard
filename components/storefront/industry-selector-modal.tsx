...
const industries = [
  {
    ...
    name: "Grocery Store",
    icon: Store,
    description: "Supermarkets, convenience stores, and specialized food retailers.",
    recommended: true,
  },
  {
    id: "pharmacy",
    ...
    icon: Pill,
    description: "Pharmacies, clinics, and health supplement stores.",
    recommended: false,
  },
  {
    id: "electronics",
    name: "Electronics",
    icon: Laptop,
    description: "Consumer electronics, gadgets, and home appliance stores.",
    recommended: false,
  },
  {
    id: "fashion",
    ...
    icon: Shirt,
    description: "Clothing, footwear, and accessory boutiques.",
    recommended: false,
  },
  {
    id: "restaurant",
    ...
    icon: Utensils,
    description: "Dine-in, takeout, and coffee shop establishments.",
    recommended: false,
  },
]

export function IndustrySelectorModal() {
  const [isOpen, setIsOpen] = useState(false)
  const [mounted, setMounted] = useState(false)

  useEffect(() => {
    setMounted(true)
    const selectedIndustry = localStorage.getItem("storefront-industry")
    if (!selectedIndustry) {
  ...
  const handleSelect = (id: string) => {
    localStorage.setItem("storefront-industry", id)
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-gray-50/80 backdrop-blur-md p-4 sm:p-6 overflow-y-auto">
          <motion.div
            initial={{ opacity: 0, scale: 0.95, y: 20 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.95, y: 20 }}
            transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }}
            className="w-full max-w-5xl bg-white rounded-3xl shadow-2xl border border-gray-100 my-auto"
          >
                    ...
                    <motion.div
                      ...
                      className={`group relative p-6 rounded-2xl border-2 transition-all cursor-pointer ${
                        industry.recommended 
                          ? "border-rose-200 bg-rose-50/50 hover:bg-rose-50 hover:border-rose-400 hover:shadow-lg hover:shadow-rose-100" 
                          : "border-gray-100 hover:border-gray-300 hover:bg-gray-50 hover:shadow-md"
                      }`}
                    >
                      {industry.recommended && (
