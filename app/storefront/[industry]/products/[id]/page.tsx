...
export default async function ProductDetailsPage({
  ...
  params: Promise<{ id: string, industry: string }>
}) {
  ...
  if (industry !== 'grocery') {
    const { getMockProduct, getMockProducts } = await import("@/lib/storefront-mock-data");
    product = getMockProduct(industry, id);
    if (product) {
      product.is_active = true;
      const allMocks = getMockProducts(industry, product.category_id);
      relatedProducts = allMocks.filter((p: any) => p.id !== id).slice(0, 4);
    }
  } else {
    try {
